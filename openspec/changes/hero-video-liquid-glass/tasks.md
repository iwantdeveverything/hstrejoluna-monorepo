...
- [ ] 7.1 Update `e2e/hero.spec.ts` — video contract in live DOM; poster painted with zero `**/*.{webm,mp4}` requests before idle (route interception).
- [ ] 7.2 Update `e2e/hero.reduced-motion.spec.ts` — `emulateMedia` reduce → poster only, no media requests, no canvas.
- [ ] 7.3 New `e2e/hero.tiers.spec.ts` — mobile → no canvas + SVG filter; desktop → canvas; `?forceWebGL=true` on degraded context → no canvas; kill-switch smoke.
- [ ] 7.4 New `e2e/hero.contrast.spec.ts` — seek 2 fixed timestamps (brightest/darkest), text contrast ≥4.5:1 over glass with grain overlay active.
- [ ] 7.5 Confirm axe spec green; route-away unmount logs no console errors (dispose verified at unit level only — no `gc()` in Playwright).
