...
> **Note**: Design chose `revalidate = 3600` (1 hour) as authoritative over the original spec's `revalidate = 60`. Rationale: portfolio content changes weekly at most; 1h bounds stale-data risk within a single editing session while eliminating 99.97% of origin hits during CI runs.

## Requirements

### Requirement: ISR Replaces force-dynamic

The home page SHALL use `export const revalidate = 3600` instead of `force-dynamic`.

#### Scenario: cached page served from CDN within window

- GIVEN a page was built and cached by the CDN
- WHEN a request arrives within the 3600s revalidate window
...
- GIVEN the 3600s revalidate window has elapsed since last generation
...
- AND stale-by-≤3600s data SHALL be acceptable for public read paths
